...
from ...account import CustomerEvents as account_events
...
class StaffDeleteMixin(UserDeleteMixin):
    ...
    def clean_instance(cls, info, instance):
        ...
        cls.check_if_requestor_can_manage_users(
    ...
    def check_if_users_can_be_deleted(cls, info, instances, field, errors):
        ...
        if not_staff_users:
            ...
            errors[field].append(ValidationError(
                msg, code=code, params=params))
...
def get_allowed_fields_camel_case(allowed_fields: set) -> set:
    ...
    fields = {validation_field_to_camel_case(
...
def get_out_of_scope_permissions(
    requestor: Union["User"], permissions: List[str]
) -> List[str]:
